...
function EditAvatarPopup({ isOpen, onClose, onUpdateAvatar, isLoading }) {
  ...
  React.useEffect(() => {
    if (/^https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z0-9]+\.[^\s]{2,}$/.test(inputLinkAvatar.current.value)) {
      setIsDisabled(false);
    } else {
      setIsDisabled(true);
    }
